...
<div class="d-flex" style="min-height: 100vh; background-color: #f4f6fa; margin-top: -24px;">
    <div class="bg-dark text-white p-3 shadow-lg" style="width: 270px;">
        ...
        <div class="nav flex-column gap-2 mt-4">
            ...
            <a href="public_entry.php?url=admin-products" class="nav-link text-white sidebar-link active"><i class="fas fa-box-open me-2"></i> Quản lý Sản phẩm</a>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 bg-white p-3 rounded-4 shadow-sm border-start border-5 border-success">
            ...
            <div class="d-flex gap-3">
                <form action="public_entry.php" method="GET" class="d-flex">
                    ...
                    <input type="text" name="q" class="form-control me-2" placeholder="Tên sản phẩm..." value="<?= htmlspecialchars($_GET['q'] ?? '') ?>" style="width: 250px;">
                    <button type="submit" class="btn btn-secondary text-white fw-bold"><i class="fas fa-search"></i></button>
                </form>
        ...
        <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                        ...
                        <tr>
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    <img src="<?= htmlspecialchars($firstImage) ?>" class="rounded me-3 border" width="50" height="50" style="object-fit:cover;">
                                    <div>
                            ...
                            <td><?= $p['stock_count'] ?> / <span class="text-success fw-bold"><?= $p['sold_count'] ?></span></td>
            ...
            <div class="card-footer bg-white p-3 d-flex justify-content-end border-top">
                <nav><ul class="pagination mb-0">
                    <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>"><a class="page-link" href="?url=admin-products&page=<?= $page - 1 ?><?= isset($_GET['q']) ? '&q='.$_GET['q'] : '' ?>">Trước</a></li>
                    <?php for($i = 1; $i <= $totalPages; $i++): ?>
                        <li class="page-item <?= ($page == $i) ? 'active' : '' ?>"><a class="page-link" href="?url=admin-products&page=<?= $i ?><?= isset($_GET['q']) ? '&q='.$_GET['q'] : '' ?>"><?= $i ?></a></li>
                    <?php endfor; ?>
                    <li class="page-item <?= ($page >= $totalPages) ? 'disabled' : '' ?>"><a class="page-link" href="?url=admin-products&page=<?= $page + 1 ?><?= isset($_GET['q']) ? '&q='.$_GET['q'] : '' ?>">Sau</a></li>
                </ul></nav>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div class="modal-content border-0 shadow">
            ...
            <form action="public_entry.php?url=admin-product-store" method="POST" enctype="multipart/form-data">
                <div class="modal-body p-4 row g-3">
                    <div class="col-md-8">
                        <label class="fw-bold small text-muted">Tên sản phẩm</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">Giá bán (VNĐ)</label>
                        <input type="number" name="price" class="form-control" required>
                    </div>
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">Giá cũ (Thị trường)</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">Số lượng kho</label>
                        <input type="number" name="stock_count" class="form-control" value="100">
                    </div>
                    <div class="col-md-8">
                        <label class="fw-bold small text-muted">Hình ảnh (Thumbnail)</label>
                        <input type="file" name="thumbnail" class="form-control" accept="image/*" required>
                    </div>
                    <div class="col-12">
                        <label class="fw-bold small text-muted">Mô tả sản phẩm</label>
                        <textarea name="description" class="form-control" rows="4"></textarea>
                    </div>
                </div>
                <div class="modal-footer border-0 p-4 pt-0">
                    <button type="submit" class="btn w-100 fw-bold btn-success shadow-sm">Tạo Sản Phẩm</button>
                </div>
            </form>
        </div>
    </div>
            ...
            <form action="public_entry.php?url=admin-product-update" method="POST" enctype="multipart/form-data">
                <div class="modal-body p-4 row g-3">
                    ...
                    <div class="col-12 text-center mb-2">
                        <img id="e_thumb_preview" src="" width="120" class="rounded border shadow-sm" style="object-fit: contain; max-height: 120px;">
                    </div>
                    <div class="col-md-8">
                        <label class="fw-bold small text-muted">Tên sản phẩm</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">Giá cũ</label>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-muted">Số lượng kho</label>
                        <input type="number" name="stock_count" id="e_stock" class="form-control">
                    </div>
                    <div class="col-md-8">
                        <label class="fw-bold small text-muted">Đổi hình ảnh mới (Bỏ trống nếu giữ nguyên)</label>
                ...
                <div class="modal-footer border-0 p-4 pt-0">
                    <button type="submit" class="btn w-100 fw-bold btn-warning shadow-sm">Lưu Thay Đổi</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    ...
    // Xử lý nạp dữ liệu vào form Edit
    document.querySelectorAll('.btn-edit-product').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // Xử lý hiển thị ảnh (lấy ảnh đầu tiên nếu là mảng)
            let rawThumb = this.dataset.thumb;
            let firstImg = rawThumb.split(',')[0].trim();
            document.getElementById('e_thumb_preview').src = firstImg;
    ...
    function deleteProduct(id) {
        Swal.fire({
            ...
            text: "Hành động này sẽ xóa dữ liệu vĩnh viễn khỏi CSDL!", 
            icon: 'error',
            ...
            if (result.isConfirmed) {
                window.location.href = `public_entry.php?url=admin-product-delete&id=${id}`;
            }
        });
    }
</script>